    ...
    public class BigSetGenericKVClient
    {
        public static void Main()
        {
            try
            {
                ...
                Console.WriteLine("clientInfo = "+clientInfo);
                Console.WriteLine(clientInfo.isOpen());
                if (!clientInfo.isOpen())
                {
                    clientInfo.doOpen();
                }

                Console.WriteLine(clientInfo.sureOpen());
                //var aRes;
                var tmp = clientInfo.getClient();
                Console.WriteLine("tmp = "+tmp);
                TStringBigSetKVService.Client aClient = (TStringBigSetKVService.Client) tmp;
                try
                {
                    var aRes = aClient.bsGetItemAsync("EmailTable", Encoding.ASCII.GetBytes("0"));
                    Console.WriteLine("get slice value  = " + Encoding.UTF8.GetString(aRes.Result.Item.Value));
                }
                catch (Exception e)
                {
                    Console.WriteLine("except get item");
                }
            ...
            //     TTransport transport = new TSocketTransport(ipaddress, 18407, 100000);
            ...
            //     aa = protocol;
            //     Console.WriteLine("aa = "+aa.GetType().Name);
            ...
            //         var bsGetItemAsync = client.bsGetItemAsync("EmailTable", Encoding.ASCII.GetBytes("1"));    
